...
namespace ThoughtWorks
{
    class Challenge4 : BaseApi
    {
        ...
        public IRestResponse PostChallenge(string postUrl)
        {
            ...
            request.AddJsonBody(new { output = new { totalValue = currentElemsPrice } });

            //execute the request to client
            return Client.Execute(request);
        }

        public int GetTotalPriceCurrentProduct()
        {
            int totalPrice = 0;
